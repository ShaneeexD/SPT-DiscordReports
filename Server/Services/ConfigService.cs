...
public sealed class ConfigService(Log log)
{
    ...
    public bool IsScreenshotEnabled(WebhookDestination destination, Events.RaidEventType type)
    {
        ...
        return s.Enabled && type switch { Events.RaidEventType.Death => s.DeathScreenshots, Events.RaidEventType.Extract => s.ExtractScreenshots, Events.RaidEventType.Loot => s.RareLootScreenshots, Events.RaidEventType.Quest => s.QuestScreenshots, Events.RaidEventType.BossKill => s.BossKillScreenshots, _ => false };
    }
